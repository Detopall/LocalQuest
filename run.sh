...
function show_help() {
    echo "Usage: $0 [OPTION]"
    echo "Manage the Docker Compose services for local-quest."
    echo
    echo "Options:"
...
function start_services() {
  ...
  if [[ -z "$(docker ps -q -f name=localquest-server)" || -z "$(docker ps -q -f name=localquest-client)" ]]; then
    echo "Containers not running. Checking for images..."

    # Check if images exist
    if [[ -z "$(docker images -q localquest-server:latest)" || -z "$(docker images -q localquest-client:latest)" ]]; then
