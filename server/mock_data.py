...
TOPICS = ["Technology", "Gardening", "Finance", "Baby Sitting", "Pet Sitting", "House Sitting", "Cooking", "Tutoring", "Cleaning", "Other"]
...
def create_quest(auth_token, user_id):
	...
	response = requests.post(f"{BASE_URL}/quests", json=quest, cookies={"auth_token": auth_token})
	response.raise_for_status()
	return response.json()["quest"]

def apply_to_quest(auth_token, quest_id):
	response = requests.post(f"{BASE_URL}/quests/{quest_id}/apply", cookies={"auth_token": auth_token})
	response.raise_for_status()
...
def main():
	...
	for user in USERS:
		...
		created_quests = [create_quest(auth_token, user_id) for _ in range(random.randint(2, 4))]
	...
	response = requests.get(f"{BASE_URL}/quests", cookies={"auth_token": first_user_auth_token})
	response.raise_for_status()
