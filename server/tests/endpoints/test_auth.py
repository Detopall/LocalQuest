...
def test_create_user(client):
    ...
    response = client.post(
        "/auth", json={"username": username, "password": password, "email": email}
    )
...
def test_login_with_wrong_password(client, test_db):
    ...
    response = client.post(
        "/auth", json={"username": username, "password": wrong_password}
    )
...
def test_logout(client, test_db):
    ...
    assert (
        test_db["cookies"].find_one({"username": username, "cookie": auth_token})
        is None
    )
