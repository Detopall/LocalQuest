...
def create_quests(test_db, creator_id=None, applicants=None):
    ...
    test_db["topics"].insert_many(
        [{"_id": topic1_id, "name": "topic1"}, {"_id": topic2_id, "name": "topic2"}]
    )
...
def test_delete_user_removes_applicant_from_quests(client, test_db):
    ...
    test_db["users"].insert_one(
        {"_id": user_id, "name": "Test User", "email": "test@example"}
    )

    quest1_id, quest2_id = create_quests(
        test_db, applicants=[auth_user_id], creator_id=user_id
    )
