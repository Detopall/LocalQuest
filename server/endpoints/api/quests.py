...
async def create_quest(user_quest: Quest, request: Request, db: sqlite3.Connection = Depends(get_db_connection)):
	...
	user = crud.get_user_by_username(db, user_quest.username)
	if not user:
		return JSONResponse(content={"message": "User not found"}, status_code=404)

	quest = crud.create_quest(db, title=user_quest.title, description=user_quest.description, topic_names=user_quest.topics, user_id=user["id"], longitude=user_quest.longitude, latitude=user_quest.latitude, deadline=user_quest.deadline)
